"""
DDP training with TensorBoard integration
"""
import os
import logging
import argparse
...
def train_epoch(model, train_loader, optimizer, rank, epoch, sampler, writer):
    ...
    for batch in tqdm(train_loader, desc=f"Rank {rank} - Epoch {epoch}", disable=(rank != 0)):
        step += 1
        images, masks = batch["image"].to(rank), batch["mask"].to(rank)
        optimizer.zero_grad()
        loss = monai.losses.DiceLoss(sigmoid=True)(model(images), masks)
        loss.backward()
        optimizer.step()
...
def validate(model, val_loader, rank, epoch, writer):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(val_loader, desc=f"Rank {rank} - Validation", disable=(rank != 0))):
            images, masks = batch["image"].to(rank), batch["mask"].to(rank)
            outputs = post_trans(model(images))
            dice_metric(y_pred=outputs, y=masks)
            if idx == 0 and rank == 0:
                first_batch = (images, masks, outputs)
    metric_tensor = torch.tensor(dice_metric.aggregate().item()).to(rank)
    dice_metric.reset()
    dist.all_reduce(metric_tensor, op=dist.ReduceOp.AVG)
    if rank == 0:
        writer.add_scalar("val/dice", metric_tensor.item(), epoch)
        if first_batch:
            plot_2d_or_3d_image(Transpose((0, 1, 4, 3, 2))(first_batch[0]), epoch, writer, index=0, tag="val/image")
            plot_2d_or_3d_image(Transpose((0, 1, 4, 3, 2))(first_batch[1]), epoch, writer, index=0, tag="val/mask")
            plot_2d_or_3d_image(Transpose((0, 1, 4, 3, 2))(first_batch[2]), epoch, writer, index=0, tag="val/prediction")
...
def ddp_main(rank, world_size, args):
    ...
    writer = SummaryWriter(log_dir=args.out_dir) if rank == 0 else None
    train_ds, train_loader, val_ds, val_loader, train_sampler = create_data_loaders(args.data, args.batch_size, rank, world_size)
    model = DDP(ModelDefinition(args.model_name).get_model().to(rank), device_ids=[rank])
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-5)
    best_metric = -1
    for epoch in range(1, args.epochs + 1):
        train_epoch(model, train_loader, optimizer, rank, epoch, train_sampler, writer)
        if epoch % 2 == 0:
