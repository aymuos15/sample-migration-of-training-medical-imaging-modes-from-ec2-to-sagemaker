"""
DDP training with MLFlow integration
"""
import os
import logging
import argparse
...
def validate(model, val_loader, rank, epoch):
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"Rank {rank} - Validation", disable=(rank != 0)):
            images, masks = batch["image"].to(rank), batch["mask"].to(rank)
            dice_metric(y_pred=post_trans(model(images)), y=masks)
...
def ddp_main(rank, world_size, args):
    ...
    if rank == 0:
        ...
        mlflow.start_run()
        mlflow.log_params({"model_name": args.model_name, "batch_size": args.batch_size, 
                          "epochs": args.epochs, "lr": args.lr, "world_size": world_size})
    ...
    for epoch in range(1, args.epochs + 1):
        train_epoch(model, train_loader, optimizer, rank, epoch, train_sampler)
        if epoch % 2 == 0:
